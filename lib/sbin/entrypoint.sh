...
template $ACCUMULO_CONF_DIR/accumulo.properties
template $ACCUMULO_CONF_DIR/client.conf
template $ACCUMULO_CONF_DIR/accumulo-client.properties
    ...
    "master" | "tserver" | "monitor" | "gc" | "tracer")
      ...
      if [[ ($ROLE = "master") ]]; then
        if accumulo_instance_exists $INSTANCE_NAME $ZOOKEEPERS; then
          ...
          runuser -p -u $USER -- accumulo init --password ${ACCUMULO_PASSWORD} --instance-name ${INSTANCE_NAME}
      ...
      #source /opt/geomesa/bin/setup-namespace.sh -u root -n geomesa -g /opt/geomesa/dist/accumulo/geomesa-accumulo-distributed-runtime_$GEOMESA_VERSION.jar
      fi
